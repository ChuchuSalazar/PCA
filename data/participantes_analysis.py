...
def generar_datos_participantes_simulados():
    ...
    data = {
        ...
        'PCA1': np.random.choice([1, 2], n_participantes, p=[0.45, 0.55]),  # Sexo: más mujeres
        'PCA2': np.random.choice(range(1, 10), n_participantes, p=[0.05, 0.15, 0.20, 0.25, 0.15, 0.10, 0.05, 0.03, 0.02]),  # Edad
        'PCA4': np.random.choice(range(1, 7), n_participantes, p=[0.02, 0.08, 0.15, 0.45, 0.25, 0.05]),  # Estudios
        'PCA5': np.random.choice(range(1, 7), n_participantes, p=[0.20, 0.30, 0.25, 0.15, 0.08, 0.02]),  # Ingresos
        'PCA6': np.random.choice([1, 2], n_participantes, p=[0.35, 0.65]),  # Ahorro
        'PCA7': np.random.choice([1, 2], n_participantes, p=[0.40, 0.60])   # Estado civil
    }
...
def create_composition_chart(df):
    ...
    df_comp['Estado_Civil'] = df_comp['PCA7'].map({1: 'Soltero/Div/Viudo', 2: 'Casado/U.Libre'})
    
    # Contar combinaciones
    combinaciones = df_comp.groupby(['Sexo', 'Ahorra', 'Estado_Civil']).size().reset_index(name='count')
    
    fig = go.Figure(go.Sunburst(
        ids=[f"{row['Sexo']}", 
             f"{row['Sexo']} - {row['Ahorra']}", 
             f"{row['Sexo']} - {row['Ahorra']} - {row['Estado_Civil']}"],
        labels=[row['Sexo'], 
                f"{row['Ahorra']}", 
                f"{row['Estado_Civil']}"],
        parents=["", 
...
def mostrar_distribuciones_detalladas(df):
    ...
    mapeos = {
        ...
        'PCA2': {1: '<26', 2: '26-30', 3: '31-35', 4: '36-40', 5: '41-45', 6: '46-50', 7: '51-55', 8: '56-60', 9: '>60'},
        'PCA4': {1: 'Primaria', 2: 'Bachillerato', 3: 'TSU', 4: 'Universitario', 5: 'Postgrado', 6: 'Doctorado'},
        'PCA5': {1: '$3-100', 2: '$101-450', 3: '$451-1800', 4: '$1801-2500', 5: '$2501-10000', 6: '+$10000'},
    ...
    fig = make_subplots(
        rows=2, cols=3,
        subplot_titles=[titulos[col] for col in ['PCA1', 'PCA2', 'PCA4', 'PCA5', 'PCA6', 'PCA7']],
    ...
    for var, pos, color in zip(variables, posiciones, colores):
        ...
        fig.add_trace(
            go.Bar(x=labels, y=freq.values, marker_color=color, name=titulos[var], showlegend=False),
            row=pos[0], col=pos[1]
        )
    
    fig.update_layout(height=600, title_text="Distribuciones Sociodemográficas de Participantes")
...
def mostrar_analisis_cruzado(df):
    ...
    df_analysis['Ingresos'] = df_analysis['PCA5'].map({
        1: '<$100', 2: '$101-450', 3: '$451-1800', 
        4: '$1801-2500', 5: '$2501-10000', 6: '+$10000'
    })
    
    # Tabla cruzada
    tabla_cruzada = pd.crosstab([df_analysis['Sexo'], df_analysis['Ahorra']], 
                               df_analysis['Ingresos'], margins=True)
...
def mostrar_tablas_detalladas(df):
    ...
    with col_filter2:
        ahorro_filter = st.selectbox("Filtrar por ahorro:", ["Todos", "Sí ahorra", "No ahorra"])
    
    with col_filter3:
        estudios_filter = st.selectbox("Estudios mínimos:", ["Todos", "Universitario+", "Postgrado+"])
    ...
    if not df_filtered.empty:
        ...
        df_display['Edad'] = df_display['PCA2'].map({1: '<26', 2: '26-30', 3: '31-35', 4: '36-40', 5: '41-45', 6: '46-50', 7: '51-55', 8: '56-60', 9: '>60'})
        df_display['Estudios'] = df_display['PCA4'].map({1: 'Primaria', 2: 'Bachillerato', 3: 'TSU', 4: 'Universitario', 5: 'Postgrado', 6: 'Doctorado'})
        df_display['Ingresos'] = df_display['PCA5'].map({1: '$3-100', 2: '$101-450', 3: '$451-1800', 4: '$1801-2500', 5: '$2501-10000', 6: '+$10000'})
        df_display['Ahorra'] = df_display['PCA6'].map({1: 'No', 2: 'Sí'})
        df_display['Estado_Civil'] = df_display['PCA7'].map({1: 'Soltero/Div/Viudo', 2: 'Casado/U.Libre'})
        
        # Mostrar tabla interpretable
        display_cols = ['Item', 'Sexo', 'Edad', 'Estudios', 'Ingresos', 'Ahorra', 'Estado_Civil']
        ...
        with col_stat3:
            ingreso_modal = df_filtered['PCA5'].mode().iloc[0] if not df_filtered['PCA5'].mode().empty else 1
            mapeo_ingresos = {1: '$3
